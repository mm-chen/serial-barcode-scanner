...
(
         ...
         FROM
            restock
         WHERE
            restock.product = products.id
            AND restock.timestamp <= sales.timestamp )
         ELSE
(
            SELECT
               CASE
...
WHERE
   deprecated = 0
   OR amount != 0;
CREATE VIEW IF NOT EXISTS purchaseprices AS
SELECT
   product,
   ...
   SELECT
      ...
      WHERE
         ...
         SELECT
            SUM(amount)
         FROM
            cashbox_diff ) ) AS amount;
CREATE VIEW IF NOT EXISTS singleDaysSinceFirstSale AS WITH RECURSIVE singledays(x) AS
(
   SELECT
(
      SELECT
         ...
         where
            s.user >= 0
         order by
            s.timestamp asc limit 1), 'unixepoch', 'start of year'))*1)
         UNION ALL
         SELECT
            x + 86400
         FROM
            singledays LIMIT 10000
)
SELECT
   x as start,
   (
...
SELECT
   ...
   (
      ...
      from
         salesView s1
      where
         s1.timestamp >= sdsp.start
         and s1.timestamp <= sdsp.
         end
         and p.id = s1.productId
   )
   as numOfProducts,
   (
      select
      ...
      from
         salesView s2
      where
         s2.timestamp >= sdsp.start
         and s2.timestamp <= sdsp.
         end
         and p.id = s2.productId
   )
   as total,
   p.name product,
   p.id productId,
